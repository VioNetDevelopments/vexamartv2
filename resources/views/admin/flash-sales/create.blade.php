...
@section('page-title', 'Buat Flash Sale')
    ...
    <div
        ...
        <div class="max-w-4xl mx-auto space-y-6">
            ...
            <div class="flex items-center gap-4 animate-fade-in-down">
                ...
                <div>
                    <h1 class="text-3xl font-black text-navy-900 dark:text-white tracking-tight">Buat Flash Sale</h1>
                    <p class="text-slate-500 dark:text-slate-400">Penawaran waktu terbatas</p>
                </div>
            </div>

            <!-- Form -->
            <form action="{{ route('admin.flash-sales.store') }}" method="POST" 
                @submit.prevent="notify.confirm('Mau launching flash sale baru nih KING? Udah bener semua datanya?', () => $el.submit())"
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="3"
                                class="w-full rounded-xl border border-slate-200 dark:border-white/10 px-4 py-3 text-sm font-medium focus:border-red-500 focus:ring-4 focus:ring-red-500/20 dark:bg-navy-800 dark:text-white resize-none"
                                placeholder="Deskripsi flash sale (opsional)">{{ old('description') }}</textarea>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <div class="relative">
                                    <input type="number" name="discount_percentage"
                                        value="{{ old('discount_percentage', 20) }}" min="1" max="90" required
                                        class="w-full rounded-xl border border-slate-200 dark:border-white/10 px-4 py-3 text-sm font-medium focus:border-accent-500 focus:ring-4 focus:ring-accent-500/20 dark:bg-navy-800 dark:text-white">
                            ...
                            <div>
                                <label
                                    ...
                                    Stok *</label>
                                <input type="number" name="max_quantity" value="{{ old('max_quantity', 100) }}" min="1"
                                    required
                                    class="w-full rounded-xl border border-slate-200 dark:border-white/10 px-4 py-3 text-sm font-medium focus:border-accent-500 focus:ring-4 focus:ring-accent-500/20 dark:bg-navy-800 dark:text-white">
                                @error('max_quantity')
                                    <p class="mt-1 text-xs text-danger">{{ $message }}</p>
