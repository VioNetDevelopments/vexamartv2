...
<body onload="window.print()">
    ...
    <div class="header text-center">
        <div class="store-name upper">{{ \App\Models\Setting::get('store_name', 'VexaMart') }}</div>
        @php
            $address = \App\Models\Setting::get('store_address', '');
            $phone = \App\Models\Setting::get('store_phone', '');
            $footer = \App\Models\Setting::get('receipt_footer', 'Terima kasih atas kunjungan Anda!');
        @endphp
        @if($address)
            <div class="store-info">{{ $address }}</div>
        @endif
        @if($phone)
            <div class="store-info">TELP: {{ $phone }}</div>
        @endif
    </div>
    ...
    <div class="info-row">
        ...
        <span>Kasir: {{ strtoupper($transaction->user->name ?? 'SYSTEM') }}</span>
    </div>
    <div class="info-row">
    ...
    <table class="items-table">
        @foreach($transaction->items as $item)
            <tr class="item-row">
                <td colspan="2">
                    <span class="item-name">{{ strtoupper($item->product->name ?? 'PRODUK TERHAPUS') }}</span>
    ...
    <div class="footer text-center">
        <div class="bold">{{ strtoupper($footer) }}</div>
    ...
    <div class="qr-code text-center">
        <img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data={{ urlencode($transaction->invoice_code) }}" alt="QR Code">
    </div>
    <div class="text-center" style="font-size: 7px; margin-top: -5px;">Scan untuk detail transaksi</div>
